...
def get_provinces(region_code=None):
    ...
    if response.status_code == 200:
        ...
        if region_code:
            data = [province for province in data if province.get('regionCode') == region_code]
        # Sort by name
        data.sort(key=lambda x: x['name'])
        return data
    return []
...
def get_cities(province_code=None):
    ...
    if response.status_code == 200:
        ...
        if province_code:
            data = [city for city in data if city.get('provinceCode') == province_code]
        # Sort by name
        data.sort(key=lambda x: x['name'])
        return data
    return []
...
def get_municipalities(province_code=None):
    ...
    if response.status_code == 200:
        ...
        # Filter by province if provided
        if province_code:
            data = [municipality for municipality in data if municipality.get('provinceCode') == province_code]
        # Sort by name
        data.sort(key=lambda x: x['name'])
        return data
    return []
...
def get_barangays(city_code=None, municipality_code=None):
    ...
    if response.status_code == 200:
        data = response.json()
        # Filter by city or municipality if provided
        if city_code:
            data = [barangay for barangay in data if barangay.get('cityCode') == city_code]
        elif municipality_code:
            data = [barangay for barangay in data if barangay.get('municipalityCode') == municipality_code]
        # Sort by name
        data.sort(key=lambda x: x['name'])
        return data
    return []
